...
# Last modified: Wed 08 Jan 2014 12:15:28 PM CST 
...
__date__ = "$Date: 2010-09-08 00:07:18 -0500 (Wed, 08 Sep 2010) $".split()[ 1 ]
...
#   for x in [x for x in wpt ]:
    for wpt_child in list( wpt ):
        if wpt_child.tag == TIME_TAG: continue
        tag = wpt_child.tag[ wpt_child.tag.find('}') + 1 : ]
        xdata.append( Data( tag, wpt_child.text ) )
...
def child_waypoint_balloon_style():
    """create and return a Style element suitable for use by a child waypoint"""
