...
q = ['zD', 'zJ']
r = ['zD', 'zE']
zones = {'zA':zA, 'zB':zB, 'zC':zC, 'zD':zD, 'zE':zE, 'zF':zF, 'zG':zG, 'zH':zH, 'zI':zI, 'zJ':zJ, 'zK':zK}
#zones = {'zA':zA} #debug
bridges = {'a':a, 'b':b, 'c':c, 'd':d, 'e':e, 'f':f, 'g':g, 'h':h, 'i':i, 'j':j, 'k':k, 'l':l, 'm':m, 'n':n, 'o':o, 'p':p, 'q':q, 'r':r}
# usedBridges & usedZones: 1 not yet used, 0 used
usedBridges = {'a':1, 'b':1, 'c':1, 'd':1, 'e':1, 'f':1, 'g':1, 'h':1, 'i':1, 'j':1, 'k':1, 'l':1, 'm':1, 'n':1, 'o':1, 'p':1, 'q':1, 'r':1}
usedZones = {'zA':1, 'zB':1, 'zC':1, 'zD':1, 'zE':1, 'zF':1, 'zG':1, 'zH':1, 'zI':1, 'zJ':1, 'zK':1}
...
def exploreBridges (zone, firstBranch):
	...
	for bridge in usableBridges:
		...
		elif ((bridges[bridge][0] == zone) and (not usedZones[bridges[bridge][1]])) or ((bridges[bridge][1] == zone) and (not usedZones[bridges[bridge][0]])):
			newUsableBridges.remove(bridge)
	...
	if len(usableBridges) > 0:
		...
		if len(path) > 1:
			...
			usedZones[ path[ len(path)-1 ] ] = 1 # reset last zone visited to not used
			# remove last zone from path
			removed = path.pop()
			debug( "remove " + removed )
			# remove last bridge crossed
			usedBridges[ path[len(path)-1] ] = 1 # reset last bridge passed to not used
			removed = path.pop()
			debug( "remove " + removed )
	...
	# else, we are in a dead end
		...
		if zonesCount == 11:
			# Test for Hamiltonian Cycle - Hamiltonian path that ends at the same zone where it started
			lastZone = path[ len(path)-1 ]
			firstZone = path[0]
			lastZoneBridges = zones[lastZone]
			for bridge in lastZoneBridges:
				bridgeZones = bridges[bridge]
				if (bridgeZones[0] == firstZone) or (bridgeZones[1] == firstZone):
					print "Hamiltonian Cycle found!"
					sys.exit()
				else:
					print "Test for Hamiltonian Cycle returned negative"
					if toFile:
						file.write ("Test for Hamiltonian Cycle returned negative\n")
